...
public class UserDao {
	...
	public void insert(UserBean user) {
		...
		jdbcTemplate.update("insert into user(name,email,gender,hobby,password,filename) values(?,?,?,?,?,?)",user.getName(),user.getEmail(),user.getGender(),hob,user.getPassword(),user.getFilename());
	}
...
//		return jdbcTemplate.update("update user set name="+user.getName() + ",email="+user.getEmail()+",gender="+user.getGender()+",hobby="+ user.getHobby()+",password="+user.getPassword()+"");
		return jdbcTemplate.update("update user set name=?,email=?,gender=?,hobby=?,password=?,filename=? where id=?", user.getName(),user.getEmail(),user.getGender(),hob,user.getPassword(),user.getFilename(),user.getId());
	} 
